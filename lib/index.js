...
class Avanza extends EventEmitter {
  ...
  */
   ...
   * Get a contract note
   *
   * @param {String} accountId ID of the account on which this note lives
   * @param {String} noteId ID of the specific note to fetch
   */
  fetchNote(accountId, noteId) {
    const path = constants.paths.NOTES_PATH
      .replace('{0}', accountId)
      .replace('{1}', noteId)
    return this.call('GET', path, {}, { 'encoding': 'binary', 'headers': { 'Content-Type': 'application/pdf' } })
  }
  ...
  * @return {Promise}
  */
  call(method = 'GET', path = '', data = {}, options = {}) {
